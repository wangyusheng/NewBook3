...
import com.example.newbook4.bean.ExchangeBookOrder;
...
public class BookExchangeOrderDetial extends BaseActivity {
	private static final String TAG = "BookExchangeOrderDetial";
	private int book_id;
	...
	private ExchangeBookOrder exchangeBookOrder;

	private ImageView iv_sign;

	protected void onCreate(Bundle savedInstanceState) {
		super.onCreate(savedInstanceState);
		setContentView(R.layout.activity_exchange_bookorder);
	...
	private void setupViewComponent() {
		...
		btn_sure.setOnClickListener(new View.OnClickListener() {
			...
			@Override
			public void onClick(View v) {
				if (exchangeBookOrder == null) {
					return;
				}
				int book_id = -1;
				boolean type = false;
				if (exchangeBookOrder.release_user == baseInfo.userId) {
					book_id = exchangeBookOrder.release_book;
					type = true;
				} else if (exchangeBookOrder.obtain_user == baseInfo.userId) {
					book_id = exchangeBookOrder.obtain_book;
					type = false;
				}
				if (book_id <= 0) {
	...
	protected void loadMainData(int book_Id) {
		try {
			...
			jsonObject.put("action", "book_exchange_retrievalOrder");
			jsonObject.put("book_id", book_Id);
		...
		try {
			...
			if (errorcode == 0) {
				...
				if (resultObject.length() > 0) {
					...
					exchangeBookOrder = new ExchangeBookOrder();

					exchangeBookOrder.exchange_id = resultObject
							.getInt("exchange_id");
					exchangeBookOrder.release_user = resultObject
							.getInt("release_user");

					exchangeBookOrder.obtain_user = resultObject
							.getInt("obtain_user");

					exchangeBookOrder.release_book = resultObject
							.getInt("release_book");

					exchangeBookOrder.obtain_book = resultObject
							.getInt("obtain_book");

					exchangeBookOrder.release_msg = resultObject
							.getString("release_msg");

					exchangeBookOrder.obtain_msg = resultObject
							.getString("obtain_msg");

					exchangeBookOrder.generate_time = resultObject
							.getString("generate_time");

					exchangeBookOrder.generate_time = resultObject
							.getString("generate_time");

					exchangeBookOrder.rfinish_time = resultObject
							.getString("rfinish_time");

					exchangeBookOrder.ofinish_time = resultObject
	...
	private void displayMainData() {
		...
		tv_ordertitle.setText("订单编号:" + exchangeBookOrder.exchange_id
				+ "\n下单时间:" + exchangeBookOrder.generate_time);

		String tempTime = exchangeBookOrder.ofinish_time;
		if (TextUtils.isEmpty(tempTime.trim())) {
			tv_address1.setText("收货状态：未收货\n"
					+ getAddress(exchangeBookOrder.obtain_msg));
		} else {
		...
		tempTime = exchangeBookOrder.rfinish_time;
		if (TextUtils.isEmpty(tempTime.trim())) {
			tv_address2.setText("收货状态：未收货\n"
					+ getAddress(exchangeBookOrder.release_msg));
		} else {
		...
		loadBookData(exchangeBookOrder.obtain_book, 0);
		loadBookData(exchangeBookOrder.release_book, 1);
	}
